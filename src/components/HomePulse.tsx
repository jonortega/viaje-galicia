      ...
      <section className="grid gap-3 sm:grid-cols-[.85fr_1.15fr]">
        <div className="rounded-[1.6rem] bg-[#e4f1ec] p-5 text-[#0b3157]">
          ...
          <div className="flex items-end gap-2">
            <strong className="text-4xl font-black tracking-[-.06em]">
              {status.days}
            </strong>
            <span className="pb-1 text-lg font-bold">días</span>
            <span className="mb-1 rounded-full bg-white/70 px-2.5 py-1 text-sm font-extrabold">
              {status.hours} h
            </span>
          </div>
        ...
        <div className="rounded-[1.6rem] border border-[#e5ddcf] bg-white p-5 shadow-[0_14px_35px_rgba(26,54,75,.07)]">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-xs font-extrabold uppercase tracking-[.16em] text-[#c08a10]">
                Próximo plan
              </p>
              <h2 className="mt-1 text-xl font-black text-[#0b3157]">
                Día 1 · {firstDay.title}
              </h2>
            </div>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {previewStops.map((place) =>
              place ? (
                <span
                  key={place.id}
                  className="rounded-full bg-[#f4efe4] px-3 py-1.5 text-xs font-bold text-[#435967]"
                >
                  {place.name}
                </span>
              ) : null,
    ...
    return (
      <section className="rounded-[1.75rem] bg-[#0b3157] p-5 text-white shadow-[0_18px_45px_rgba(11,49,87,.2)] sm:p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-xs font-extrabold uppercase tracking-[.16em] text-[#f6c94c]">
              Estamos de viaje
            </p>
