      ...
      <div className='space-y-8 px-5 pt-7 sm:px-8'>
        ...
        <section aria-labelledby='bases-title'>
          <div className='mb-4 flex items-end justify-between'>
            <div>
              <p className='text-xs font-extrabold uppercase tracking-[.16em] text-[#147d76]'>Nuestro campamento</p>
          ...
          <div className='grid gap-3 sm:grid-cols-3'>
            {trip.bases.map((base, index) => (
              ...
              >
                <span className='absolute right-3 top-1 text-6xl font-black text-[#0b3157]/[.035]'>{index + 1}</span>
        ...
        >
          ...
          <span className='min-w-0 flex-1'>
            ...
            <span className='mt-0.5 block truncate text-xs text-white/60'>{trip.playlist.subtitle}</span>
          </span>
          <Icon
            name='external'
            className='mr-1 size-5 shrink-0 text-white/55 transition-transform group-hover:translate-x-0.5'
          />
        </a>
        ...
        <section className='grid grid-cols-3 overflow-hidden rounded-[1.6rem] border border-[#e5ddcf] bg-white py-5 shadow-[0_12px_30px_rgba(26,54,75,.05)]'>
          {[
            ...
            >
              ...
              <strong className='text-2xl font-black tracking-tight text-[#0b3157]'>{stat.value}</strong>
              <span className='text-[.68rem] font-bold uppercase tracking-[.12em] text-[#82909a]'>{stat.label}</span>
            </div>
          ))}
        </section>
      </div>
